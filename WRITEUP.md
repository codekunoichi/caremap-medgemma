...
**This is not just our family's problem.** Medication errors cause an estimated 2.6 million deaths annually worldwide, with low- and middle-income countries bearing the highest burden (WHO, 2017). In India, doctors see 125-175 patients per day. Families carry paper records in bags. There is no EHR infrastructure to speak of. Our user research with four healthcare professionals and caregivers surfaced the same pattern: *"I don't need more information. I need to know what to do next."* Multilingual translation via NLLB-200 after MedGemma interpretation brings the fridge sheet down to the reading level of local Ayahs — in Bengali, Hindi, or any of 10 supported languages — because plain English is still a barrier when the caregiver reads Bangla.

**A known limitation:** CareMap requires structured clinical data as input. In the US and assisted living settings, this data already exists in EHR systems. In India, where paper records dominate, data entry remains a gap. I am actively working on closing this gap through the [Amazon Nova Multimodal Challenge](https://amazon-nova.devpost.com/), where multimodal Nova models will extract medication records from pill strip images and lab results from PDF reports — turning the paper bag into structured data that CareMap can consume.
...
**Applicability beyond India.** CareMap works equally well wherever good EHR data exists. In the US, **Senior Assisted Living facilities** face the same caregiver rotation problem — CNAs and aides change shifts, residents have complex medication regimens, and handoff quality varies. With structured EHR data as input, the fridge sheet becomes a reliable reference that outlasts any individual caregiver's shift. For **provider-side triage**, large US healthcare institutions like Sharp, Scripps, and Rendr have massive volumes of HL7 ORU messages flowing through their EHR systems with dedicated departments for review. CareMap's triage modules can help these teams prioritize life-threatening scenarios first.

**Impact potential:** India has 80+ million family caregivers and a healthcare worker shortage that won't close for decades. The US has 53 million family caregivers and 28,900 assisted living facilities. CareMap targets two leverage points where AI can have outsized impact:

1. **Caregiver medication errors** — the #1 preventable cause of hospital readmission. A printable fridge sheet that survives caregiver turnover eliminates the verbal handoff problem entirely.
2. **Triage queue prioritization** — routing critical cases to the front of a 500-study radiology queue or a 1,000-message HL7 inbox means patients get treatment in hours instead of days.
...
MedGemma generates "Ask the Doctor" prompts so families know *what questions to ask* — addressing the gap Dr. Mishra identified: families don't know what they don't know. Multilingual translation via NLLB-200 supports Bengali, Hindi, and 8 other languages, preserving medication names and doses as safety-critical untranslated elements.
...
**Resilient validation:** MedGemma 1.5 sometimes omits JSON keys non-deterministically. Rather than failing, `require_keys_with_defaults()` fills missing keys with safe fallback text ("Not specified — confirm with care team"), ensuring the fridge sheet always renders.
