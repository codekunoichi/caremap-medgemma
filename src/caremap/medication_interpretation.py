...
if __name__ == "__main__":
    ...
    print(f"{'='*60}")
    print(f"V1 important_note length: {len(result_v1.get('important_note', ''))} chars")
    print(f"V2 watch_out_for length:  {len(result_v2.get('watch_out_for', ''))} chars")
    print(f"\nV1 dropped clinician notes about kidney function: ", end="")
    print("YES" if "kidney" not in result_v1.get("important_note", "").lower() else "NO")
    print(f"V2 includes kidney warning: ", end="")
    print("YES" if "kidney" in result_v2.get("watch_out_for", "").lower() else "NO")